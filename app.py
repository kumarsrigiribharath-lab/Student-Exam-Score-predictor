...
with col1:
    ...
    hours = st.number_input("Hours Studied (e.g. 5)", min_value=0, max_value=24, value=5)
    attendance = st.number_input("Attendance (%) (e.g. 80)", min_value=0, max_value=100, value=75)
    sleep = st.number_input("Sleep Hours (e.g. 6–8)", min_value=0.0, max_value=12.0, value=7.0)
    previous = st.number_input("Previous Score (e.g. 70)", min_value=0, max_value=100, value=70)
    tutoring = st.number_input("Tutoring Sessions (e.g. 2)", min_value=0, max_value=10, value=2)
    physical = st.number_input("Physical Activity (e.g. 3)", min_value=0, max_value=20, value=3)
